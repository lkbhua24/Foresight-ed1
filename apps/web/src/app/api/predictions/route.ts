...
export async function GET(request: NextRequest) {
  try {
    ...
    // 构建查询函数
    const buildQuery = (withOutcomes: boolean) => {
      let selectExpr = '*';
      if (withOutcomes) selectExpr = '*, outcomes:prediction_outcomes(*)';
      let query = client
        .from('predictions')
        .select(selectExpr)
    ...
    // 首次尝试获取数据
    let { data: predictions, error } = await buildQuery(includeOutcomes);
    
    // 如果获取失败且启用了outcomes，尝试降级获取（不带outcomes）
    if (error && includeOutcomes) {
      console.warn('带选项获取预测列表失败，尝试降级获取:', error);
      const retryResult = await buildQuery(false);
      predictions = retryResult.data;

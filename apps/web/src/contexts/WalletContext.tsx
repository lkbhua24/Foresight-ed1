...
const handleEIP6963Announce = (
  event: CustomEvent<EIP6963AnnounceDetail> | any
) => {
  const detail = event?.detail as EIP6963AnnounceDetail;
  if (!detail?.provider) return;
  const exists = discoveredProviders.some(
    (d) => d.provider === detail.provider
  );
  if (!exists) {
...
export function WalletProvider({ children }: { children: ReactNode }) {
  ...
  const detectWallets = (): WalletInfo[] => {
    ...
    if (discoveredProviders.length > 0) {
      for (const d of discoveredProviders) {
        const mapped =
          providerTypeMap.get(d.provider) || walletTypeFromInfo(d.info);
  ...
  const getProviderByType = (type: WalletType | null): any => {
    ...
    if (type === "binance" && (window as any).BinanceChain)
      return (window as any).BinanceChain;
    if (type === "coinbase" && (window as any).coinbaseWalletExtension)
    ...
    if (
      type === "metamask" &&
      ethereum &&
      identifyWalletType(ethereum) === "metamask"
    )
      return ethereum;
    return null;
  };
  ...
  const identifyWalletType = (provider?: any): WalletType | null => {
    ...
    try {
      ...
      console.debug("钱包类型识别出错:", error);
    }

    // 独立注入的钱包检查
    if ((window as any).BinanceChain === p || (window as any).BinanceChain)
      return "binance";
    if (
      (window as any).coinbaseWalletExtension === p ||
      (window as any).coinbaseWalletExtension
    )
      return "coinbase";
    if ((window as any).okxwallet === p || (window as any).okxwallet)
      return "okx";
    ...
    if (!provider && ethereum?.providers) {
      for (const pr of ethereum.providers) {
        ...
        if (pr.isMetaMask && !pr._metamask && !pr.isCoinbaseWallet)
          return "binance";
      }
    }
  ...
  useEffect(() => {
    ...
    const checkConnection = async () => {
      try {
        ...
        const lastTypeRaw =
          typeof window !== "undefined"
            ? localStorage.getItem(LAST_WALLET_TYPE)
            : null;
        const lastType =
          lastTypeRaw === "metamask" ||
          lastTypeRaw === "coinbase" ||
          lastTypeRaw === "binance" ||
          lastTypeRaw === "okx"
            ? (lastTypeRaw as WalletType)
            : null;
        if (lastType) {
          const lastProvider = getProviderByType(lastType);
          if (lastProvider && typeof lastProvider.request === "function") {
            try {
              const accs = await lastProvider.request({
                method: "eth_accounts",
              });
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          try {
            const hex = await (rawProvider as any)?.request?.({
              method: "eth_chainId",
            });
  ...
  const switchNetwork = async (
    ...
    try {
      ...
      try {
        ...
        const hexId =
          typeof net.chainId === "bigint"
            ? "0x" + net.chainId.toString(16)
            : (ethers.toBeHex as any)?.(net.chainId) ??
              "0x" + Number(net.chainId).toString(16);
      ...
      if (walletType) {
        ...
        // 在 providers 数组中查找目标钱包
          ...
          for (const provider of ethereum.providers) {
            ...
            if (
              walletType === "binance" &&
              (t === "binance" || provider.isBinanceWallet)
            ) {
              targetProvider = provider;
              break;
            }
        ...
        if (walletType === "okx") {
          ...
          if (typeof window !== "undefined") {
            ...
            if ((window as any).okxwallet) {
              ...
              // 如果仍然没有找到，抛出特定错误
              throw new Error(
                "OKX钱包未安装或未正确注入。请确保已安装OKX钱包扩展程序并刷新页面。"
              );
            }
          }
        } else {
      ...
      if (accounts && accounts.length > 0) {
        ...
        if (typeof window !== "undefined") {
          ...
          try {
            localStorage.setItem(
              LAST_WALLET_TYPE,
              String(actualWalletType || walletType || "")
            );
            localStorage.setItem(
              LAST_WALLET_ADDRESS,
              String(accounts[0]).toLowerCase()
    ...
    try {
      ...
      if (currentProvider) {
          ...
          // 或者尝试清除权限（MetaMask 等）
            ...
            try {
              ...
              try {
                ...
                console.debug("钱包不支持权限管理:", requestError);
              }
            }
          }
        } catch (disconnectError) {
          console.debug("钱包断开连接失败:", disconnectError);
        }
        ...
        try {
          ...
          console.debug("遍历 providers 撤销权限失败:", e);
        }
      ...
      if (typeof window !== "undefined") {
        ...
        try {
          ...
          console.debug("清除存储失败:", storageError);
        }
      }
  ...
  const siweLogin = async (): Promise<{
    ...
    try {
      ...
      const chainIdNum = Number(
        net?.chainId?.toString?.() || walletState.chainId || "1"
      );
  ...
  // 请求钱包权限（兼容不支持权限管理的钱包）
  const requestWalletPermissions = async (): Promise<{
    success: boolean;
    error?: string;
  }> => {
    try {
      ...
      try {
        ...
        try {
          signature = await signer.signMessage(
            JSON.stringify({ type: "Approve", ...message })
          );
        } catch (e) {
